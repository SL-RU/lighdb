...
/*
  ...
  Data file structure:
  |item's data one by one(item_size * count bytes)|
*/
...
typedef struct {
    ...
    uint32_t index_offset; //offset of ID datain file_index
    //buffers
    uint8_t *buffer1;
    uint8_t *buffer2;
    uint8_t *buffer_id;
 ...
 * @param buffer_size returns buffer size for DB functionality. You need to set buffer using ldb_set_buffer(); Size equals [(size of item) * 2 + N] bytes. N byte for buffer of indexes, more is better. But not less then 200 bytes
 ...
 * @param buffer_size returns buffer size for DB functionality. You need to set buffer using ldb_set_buffer(); Size equals [(size of data) * 2 + N] bytes. N byte for buffer of indexes, more is better. But not less then 200 bytes
 * @return result LDB_OK, LDB_ERR_IO
 */
